...
font_obj = pygame.font.Font('GenYoGothicTW-Regular.ttf', 20)	   #設定字體→(字體名稱,字體尺寸)
INITMONEY = 15000 # Money that each player gets when game starts   #遊戲會開始會得到的東西
...
def main():
	...
	location_list.append(Location(1,IMAGES['cat1'],game_map[1],FUNCTION['fortune'],3000,1500,5000))
	location_list.append(Location(2,IMAGES['cat3'],game_map[2],FUNCTION['fortune'],3000,1500,5000))
	location_list.append(Location(3,IMAGES['dog1'],game_map[3],FUNCTION['fortune'],3000,1500,5000))
	location_list.append(Location(4,IMAGES['fortune2'],game_map[4],FUNCTION['fortune'],3000,1500,5000))
	location_list.append(Location(5,IMAGES['cat2'],game_map[5],FUNCTION['fortune'],1500,900,5000))
	location_list.append(Location(6,IMAGES['cat5'],game_map[6],FUNCTION['estate'],3000,1500,5000))
	location_list.append(Location(7,IMAGES['dog2'],game_map[7],FUNCTION['estate'],1500,900,5000))
	location_list.append(Location(HOSPITAL_POS,IMAGES['hospital'],game_map[HOSPITAL_POS],FUNCTION['hospital'],-1,-1,-1)) 
	location_list.append(Location(9,IMAGES['dog3'],game_map[9],FUNCTION['estate'],1500,900,5000))
	location_list.append(Location(10,IMAGES['cat4'],game_map[10],FUNCTION['estate'],3000,1500,5000))
	location_list.append(Location(11,IMAGES['dog4'],game_map[11],FUNCTION['estate'],1500,900,5000))
	location_list.append(Location(12,IMAGES['forest1'],game_map[12],FUNCTION['attack'],-1,-1,-1))
	location_list.append(Location(13,IMAGES['cat6'],game_map[13],FUNCTION['estate'],3000,1500,5000))
	location_list.append(Location(14,IMAGES['dog5'],game_map[14],FUNCTION['estate'],1500,900,800))
	location_list.append(Location(15,IMAGES['dog6'],game_map[15],FUNCTION['estate'],3000,1500,5000))
	location_list.append(Location(16,IMAGES['cat7'],game_map[16],FUNCTION['estate'],3000,1500,5000))
	location_list.append(Location(17,IMAGES['dog8'],game_map[17],FUNCTION['estate'],1500,900,5000))
	location_list.append(Location(18,IMAGES['fortune1'],game_map[18],FUNCTION['fortune'],1500,900,5000))
	location_list.append(Location(19,IMAGES['angry1'],game_map[19],FUNCTION['estate'],9000,5000,5000))
	location_list.append(Location(20,IMAGES['camping'],game_map[20],FUNCTION['cute'],-1,-1,-1))
	location_list.append(Location(21,IMAGES['angry2'],game_map[21],FUNCTION['estate'],9000,5000,5000))
	location_list.append(Location(22,IMAGES['cat10'],game_map[22],FUNCTION['estate'],3000,1500,5000))
	location_list.append(Location(23,IMAGES['dog7'],game_map[23],FUNCTION['estate'],1500,900,5000))
			...
			if event.type == pygame.MOUSEBUTTONDOWN:
				...
				if go_btn.isHover():
					...
					if location.func == FUNCTION['attack']:							#如果玩家被攻擊的話，要重新繪製視窗(重複168行的東西)
						...
						cans=['4000','5000','6000','6000','6000','6000','6000','6000','6000']
						i=random.randint(0,8)
						Lose_Get=1#random.randint(0,1)

						if Lose_Get == 1:											#命運是不好的(lose)	
							...
							if story == 1:
								...
								moneyget.play()
								CreateMessageBox('Communism!!','Our money!',MESSAGE_BOX_TYPE['OK'])
								m=0
								for i in players:
									m+=i.money
								for i in players:
						...
						if (location.owner == -1) and (player_cur.money >= location.cost):     #判斷該地是否有所有者 & 玩家所擁有的金錢是否大於購買價，都符合的話即跳出文字視窗詢問是否要購買
							...
							if location.tolls > player_cur.money:					#路過該地的費用>玩家的錢，跳出玩家破產的文字視窗
								...
								players[location.owner - 1].money += player_cur.money 		#所有者的錢數+費用
								#player_cur.money = 0										#玩家的錢數歸0
								RecordData(players,game_record,location_list)
								for i in players:
									print('estate:',i.asset)								#紀錄玩家們的金錢變化
								player_cur,location_list=chance(player_cur,location_list,game_record,name)
																	#遊戲結束並且印出折線圖
...
def DrawPlayerData(players,name):								#利用game_class中來計算player的金錢有多少，並且在左上角將資訊繪製出來
	...
	for player in players:
		text = font_obj.render('%5s：%d'% (name[player.player_num-1],player.money),True,WHITE,(0,0,0,0))
		text_rect = text.get_rect()
...
def RecordData(players,game_record,location):		
	...
	for player in players:
		for loc in location:
			if loc.owner==player.player_num:
				player.asset+=loc.cost+(loc.buildings-1)*loc.extend_cost
	...
	if players.asset<0:
		...
		for loc in location:
			if loc.owner == players.player_num:
				players.money+=loc.buildings*loc.extend_cost+loc.cost
				loc.buildings=0
				loc.owner=-1
		if players.money<0:
			GameOver(game_record,name)
...
if __name__ == '__main__':
	...
	msg=easygui.msgbox(msg='當你走進他們的領地時，可以宣示終身照顧牠(領養)，或是俸上美味的罐頭食品作為見面禮，也可以選擇為貓貓狗狗們進行療傷，若是你什麼都不願意給牠們，小心會被咬喔!',title='寵物當家')
	'''
	main()
